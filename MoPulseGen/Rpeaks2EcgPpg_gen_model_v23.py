...
class Rpeaks2EcgPpg_Simulator(Simulator):
    ...
    def load_model_stitchGAN(self,path,w_pk,Fs_out,stateful=True):
        ...
        if Fs_out==100:
            model=self.create_stitchGAN_model(shape_in=[(None,RNN_win_len,self.n_stitch_channels)],
    ...
    def learn_ecg_stitchGAN(self,save_flag=False):
        ...
        self.ecg_stitchGAN=self.learn_stitchGAN_model(self.input[::4],
                                        ...
                                        w_pk=self.w_pk*self.ecg_by_ppg_pk,
                                        w_l=self.w_l*self.ecg_by_ppg_pk,
                                        save_flag=save_flag,EPOCHS = 100)
        return
...
# =============================================================================
    ...
    def learn_stitchGAN_model(self,input_list,output_list,path4model,
        ...
        if factr==1:
            get_intermediate_signal=self.get_intermediate_signal_ecg
        elif factr==4:
            get_intermediate_signal=self.get_intermediate_signal_ppg
        else:
            raise AssertionError('factor can be only 1 (ECG) or 4 (PPG). Check'
                                 'data dimensions')
        ...
        for i in range(len(input_list)):
            arr_y,out,_=get_intermediate_signal(input_list[i],factr,
...
# =============================================================================
    ...
    def get_intermediate_signal_ecg(self,arr_pks,factr,w_pk,w_l,out=None,model=None):
        '''
        sim_pks2sigs
        '''
        w_r=w_pk-w_l-1
        ...
        arr_y=np.zeros([int(len(arr_pks)/factr),self.n_stitch_channels])            
        #Place sampled signal peaks
        for i in range(len(r_pk_locs)):
            arr_y[r_pk_locs[i]-w_l:r_pk_locs[i]+w_r+1,int(i%self.n_stitch_channels)]=pks[i,:]
        
        clip_indices=[r_pk_locs[0]-w_l,r_pk_locs[-1]+w_r+1]
        arr_y=arr_y[r_pk_locs[0]-w_l:r_pk_locs[-1]+w_r+1]        
        if out is not None:
            out=out[r_pk_locs[0]-w_l:r_pk_locs[-1]+w_r+1].reshape(-1,1)
            return arr_y,out,clip_indices
        else:
            arr_pks_dsampled=arr_pks_dsampled[r_pk_locs[0]-w_l:r_pk_locs[-1]+w_r+1]
            return arr_y,arr_pks_dsampled,clip_indices
    
    def get_intermediate_signal_ppg(self,arr_pks,factr,w_pk,w_l,out=None,model=None):
        '''
    ...
    def __call__(self,arr_pks,sigs2return=['PPG','ECG']):
        ...
        if 'PPG' in sigs2return:
            ...
            arr_interm_ppg,arr_pks_ppg,clip_indices_ppg=(
                                        self.get_intermediate_signal_ppg(arr_pks,
                                        factr_ppg,
        ...
        if 'ECG' in sigs2return:
            ...
            arr_interm_ecg,arr_pks_ecg,clip_indices_ecg=(
                                        self.get_intermediate_signal_ecg(arr_pks,
                                        factr_ecg,
            ...
            #modify clip indices if ppg exists
            if returned_sigs[5] is not None:
                diff_start_idx=(4*clip_indices_ppg[0])-clip_indices_ecg[0]
                diff_end_idx=clip_indices_ecg[1]-(4*clip_indices_ppg[1])
                arr_ecg_stitched=arr_ecg_stitched[diff_start_idx:-diff_end_idx]
                arr_pks_ecg=arr_pks_ecg[diff_start_idx:-diff_end_idx]
                clip_indices_ecg=[4*i for i in clip_indices_ppg]
...
# =============================================================================
    ...
    with open('./figures/fig_REPv23.pickle', 'wb') as file:
