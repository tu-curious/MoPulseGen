...
class Rpeaks2EcgPpg_Simulator(Simulator):
    ...
    def create_stitchGAN_model(self,shape_in,shape_out,mode='stitch',
        ...
        #TODO Change LSTM to GRU
        lstm_out=layers.LSTM(64,return_sequences=True,stateful=stateful)(inputs)
        out= layers.Conv1D(1,1)(lstm_out)
        #out=layers.BatchNormalization()(out)
        #out=tf.keras.activations.tanh(out)
        #out=layers.Conv1D(1,1)(out)
        #out=tf.concat([out[:,:,0:1],tf.math.softplus(out[:,:,1:2])],axis=-1)
...
# =============================================================================
    ...
    def get_intermediate_signal(self,arr_pks,factr,w_pk,w_l,out=None,model=None):
        ...
        #clip terminal r_pks
        r_pk_locs_origin=r_pk_locs_origin[2:-2]
        #get nearest dsampled idx
...
#     factr_ecg=1;RNN_win_len_ecg=1000;step_size_ecg=500
...
#                                 w_pk=sim_pks2sigs.w_pk*sim_pks2sigs.ecg_by_ppg_pk,
#                                 w_l=sim_pks2sigs.w_l*sim_pks2sigs.ecg_by_ppg_pk)
#     arr_interm_ecg=sim_pks2sigs.sliding_window_fragmentation([arr_interm_ecg],
#                                     RNN_win_len_ecg,step_size_ecg)
#     
#     #Stitch them together
#     arr_ecg=sim_pks2sigs.ppg_stitchGAN.predict(arr_interm_ecg)
#     arr_ecg_stitched=np.concatenate([arr_ecg[0,:,0],
#                     arr_ecg[1:,-step_size_ecg:,0].reshape(-1)])
#     plt.figure();plt.plot(arr_ecg[0,:,0]);plt.plot(check_data_out[0,:,0])
# 
