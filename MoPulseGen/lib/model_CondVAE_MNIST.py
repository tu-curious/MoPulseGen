...
class Model_CondVAE(tf.keras.Model):
    def __init__(self,optimizer,n_units,in_shape,lamda=1):
        '''
        ...
        self.w1=tf.Variable(w_init(shape=[self.in_shape,self.n_units[cntr],],
        ...
        self.b_logsig=tf.Variable(b_init(shape=[self.n_units[cntr]],
                                   dtype=tf.float64),trainable=True)
        cntr+=1
        self.w2=tf.Variable(w_init(shape=[self.n_units[cntr-1],
                                            self.n_units[cntr]],
                                   dtype=tf.float64),trainable=True)
        ...
        self.var_list1=[self.w1,self.b1,self.w_mu,self.b_mu,
                        self.w_logsigma,self.b_logsigma,self.w2,self.b2,
                        self.w3,self.b3]
    ...
    def nn_model(self, x,cond):
        ...
        x_hat=self.sigmoid(tf.matmul(x,self.w3)+self.b3)
    ...
    def fit(self, data, summaries, epochs):
        ...
        '''
        train, val=data
        batch_size_train,N=find_batch_size(train[0].shape[0],thres=128)
        ...
        for epoch in range(epochs):
            ...
            template = ('Epoch {}, Loss1: {},Loss2: {},Test Loss1: {},'
                        'Test Loss2: {} \n')
